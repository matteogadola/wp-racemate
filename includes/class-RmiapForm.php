...
class RmiapForm {
  ...
  function __construct( $action = '', $args = false ) {
    ...
    add_shortcode('racemate-form', [$this, 'racemate_form_shortcode_handler']);

    // Registra e accoda gli stili CSS
    add_action('wp_enqueue_scripts', [$this, 'racemate_form_enqueue_styles']);
   ...
   */
   ...
   public function racemate_form_shortcode_handler( $atts, $content = null ) {
    $atts = shortcode_atts(
      array(
        'race'      => null,
        'race-id'      => null,
    ...
    /*$form = new FormBuilder(plugins_url('racemate-checkout.php', __DIR__));
    $form->lolle('name', [
      'type' => 'text',
      'placeholder' => __( 'Nome', 'wp-simple-form' ),
    ]);
    ...
    array_push($fields, '<input class="input" type="hidden" name="race_id" value="'.$atts['race'].'">');
...
array_push($fields, '<div class="cell is-col-span-2"><button class="button is-info is-light">Iscriviti</button></div>');
    ...
    $output = sprintf(
      ...
      plugins_url('racemate-checkout.php', __DIR__),
      $grid
    );
...
public function racemate_form_enqueue_styles() {
    ...
    wp_register_style(
        ...
        plugin_dir_url( __DIR__ ) . 'assets/styles/form.css', // URL del file CSS
